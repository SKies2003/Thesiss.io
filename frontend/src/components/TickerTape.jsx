...
const TickerTape = () => {
  ...
  useEffect(() => {
    const fetchTicker = async () => {
      try {
        // We use 127.0.0.1 instead of localhost to prevent IPv6 lookup issues
        ...
        if (res.ok) {
          ...
          // Silent fail in UI if backend isn't ready, just log to console
          console.warn("Ticker API not ready yet:", res.status);
        }
      } catch (e) {
        // Common error if backend is offline
        console.warn("Ticker fetch failed (Backend might be offline).");
      }
    };
